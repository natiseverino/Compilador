...
//    }
    ...
    public void invocacionID(String lexema, String uso) {
        ...
        if (token != null) {
            ...
            if (uso_real.equals(Main.PROCEDIMIENTO) && uso.equals(Main.VARIABLE)) {
                Errores.addError(String.format("[ASem] | Linea %d: No es posible realizar esa operacion utilizando el procedimiento \"%s\" %n", analizadorLexico.getNroLinea(), lexema));
            }
        }
...
//#line 319 "gramatica.y"
                {
                    Token factor = TablaSimbolos.getToken(val_peek(2).sval + "@" + getAmbitoDeclaracionID(val_peek(2).sval, Main.VARIABLE));
